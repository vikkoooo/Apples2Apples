...
public class Apples2Apples {
	...
	public Apples2Apples(int numberOfOnlinePlayers) throws Exception {
		...
		for (int i = redApples.size() - 1; i > 0; i--) {
			...
			String a = redApples.get(index);
			redApples.set(index, redApples.get(i));
			redApples.set(i, a); // SWAP
		}
		rnd = ThreadLocalRandom.current();
		for (int i = greenApples.size() - 1; i > 0; i--) {
			int index = rnd.nextInt(i + 1);
			String a = greenApples.get(index);
			greenApples.set(index, greenApples.get(i));
			greenApples.set(i, a); // SWAP
		}
		...
		while (!finished) {
			...
			for (int i = playedApple.size() - 1; i > 0; i--) {
				int index = rnd.nextInt(i + 1);
				PlayedApple a = playedApple.get(index);
				playedApple.set(index, playedApple.get(i));
				playedApple.set(i, a); // SWAP
			}
			...
			String winnerString = ((players.get(winningApple.playerID).isBot ? "Bot" : "Player") + " ID"
					+ winningApple.playerID +
			...
			if (finished) {
				String gameWinnerString = "FINISHED: "
						+ ((players.get(gameWinner).isBot ? "Bot" : "Player") + " ID" + gameWinner +
								" won the game");
